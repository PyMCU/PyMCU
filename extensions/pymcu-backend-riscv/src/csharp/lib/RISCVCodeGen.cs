...
public partial class RiscvCodeGen(DeviceConfig cfg) : CodeGen
{
    ...
    public override void Compile(ProgramIR program, TextWriter output)
    {
        ...
        // Zicsr is needed by the startup code, which programs mtvec/mepc.
        EmitRaw(".attribute arch, \"rv32ec_zicsr\"");
        EmitRaw(".attribute unaligned_access, 0");
        // ilp32e keeps the stack word-aligned; the 16-byte rule is ilp32 only.
        EmitRaw(".attribute stack_align, 4");
    ...
    private static bool IsLeaf(Function func)
    {
        foreach (var instr in func.Body)
        {
            switch (instr)
            {
                ...
                case IndirectCall:
                    return false;
                case Binary bin when bin.Op is PyMCU.IR.BinaryOp.Mul or PyMCU.IR.BinaryOp.Div
                    or PyMCU.IR.BinaryOp.FloorDiv or PyMCU.IR.BinaryOp.Mod:
                    return false;
                case AugAssign aug when aug.Op is PyMCU.IR.BinaryOp.Mul or PyMCU.IR.BinaryOp.Div
                    or PyMCU.IR.BinaryOp.FloorDiv or PyMCU.IR.BinaryOp.Mod:
                    return false;
            }
        }
    ...
    private void CompileBinary(Binary arg)
    {
        ...
        if (!usedImmediate)
        {
            ...
            switch (arg.Op)
            {
                ...
                // C division does once the operands have different signs.
