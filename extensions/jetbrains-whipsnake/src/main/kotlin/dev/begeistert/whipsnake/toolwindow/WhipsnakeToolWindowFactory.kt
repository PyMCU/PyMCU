package dev.begeistert.whipsnake.toolwindow
...
/**
 * Factory for the "Whipsnake" tool window (anchor=bottom).
 *
 ...
 */
class WhipsnakeToolWindowFactory : ToolWindowFactory {

    override fun createToolWindowContent(project: Project, toolWindow: ToolWindow) {
        val panel = WhipsnakeToolWindowPanel(project)
...
private class WhipsnakeToolWindowPanel(private val project: Project) : JPanel(BorderLayout()) {

    private val log = Logger.getInstance(WhipsnakeToolWindowPanel::class.java)

    private val chipLabel = JLabel("No Whipsnake project detected")
    ...
    private fun refreshChipLabel() {
        val config = WhipsnakeConfigReader.findConfig(project)
        if (config == null) {
            chipLabel.text = "No Whipsnake project detected"
            return
        }
    ...
    private fun runCommand(command: String) {
        ...
        ApplicationManager.getApplication().executeOnPooledThread {
            try {
                ...
                log.error("Whipsnake toolwindow command error", e)
            }
        }
    }
    ...
    private fun runSync() {
        ...
        ApplicationManager.getApplication().executeOnPooledThread {
            try {
                ...
                log.error("Whipsnake toolwindow sync error", e)
            }
        }
    }
