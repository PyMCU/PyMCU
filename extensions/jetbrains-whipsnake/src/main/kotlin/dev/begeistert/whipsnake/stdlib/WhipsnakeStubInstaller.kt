 ...
 */
object WhipsnakeStubInstaller {

    private val log = Logger.getInstance(WhipsnakeStubInstaller::class.java)
    ...
    fun install(basePath: String, stdlib: List<String>, board: String?): Path? {
        ...
        if (sitePackages != null) {
            if ("circuitpython" in stdlib) {
                changed = installPackage(sitePackages, "whipsnake_circuitpython") || changed
                changed = installBoardStub(sitePackages, board) || changed
            }
            if ("micropython" in stdlib) {
                changed = installPackage(sitePackages, "whipsnake_micropython") || changed
            }
        }
    ...
    private fun installPackage(sitePackages: Path, pkgName: String): Boolean {
        ...
        if (!pkgDir.exists()) {
            log.warn("Whipsnake stubs: $pkgName not found in $sitePackages — run sync first")
            return false
        }
        ...
        if (sourceFiles.isEmpty()) {
            log.warn("Whipsnake stubs: no public .py files found in $pkgDir")
            return false
        }
        ...
        log.info("Whipsnake stubs: generated .pyi stubs for $pkgName in $sitePackages")
        return true
    }
    ...
    private fun installBoardStub(sitePackages: Path, board: String?): Boolean {
        val pkg     = sitePackages.resolve("whipsnake_circuitpython")
        ...
        val source  = listOf(
            ...
            log.debug("Whipsnake: no board constants file found — skipping board.pyi")
            return false
        }
    ...
    private fun buildBoardStub(boardFile: Path): String {
        ...
        val sb      = StringBuilder("# Whipsnake — generated board pin stubs (do not edit manually)\n")
        val dquote3 = "\"\"\""
        sb.append("$dquote3\n")
    ...
    private fun generateDistBoard(basePath: String, board: String?, sitePackages: Path?): Boolean {
        ...
        if (source == null) {
            log.debug("Whipsnake: no board constants file found — skipping dist/_generated/board.py")
            return false
        }
        return try {
            ...
            log.info("Whipsnake: wrote dist/_generated/board.py for IDE `import board` support")
            true
        } catch (e: Exception) {
            log.warn("Whipsnake: failed to write dist/_generated/board.py", e)
            false
        }
    }
    ...
    private fun writeStub(target: Path, content: String) {
        try {
            ...
            log.warn("Whipsnake stubs: failed to write ${target.fileName}", e)
        }
    }
