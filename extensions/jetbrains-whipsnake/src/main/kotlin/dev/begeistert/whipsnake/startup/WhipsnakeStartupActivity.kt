package dev.begeistert.whipsnake.startup
...
class WhipsnakeStartupActivity : ProjectManagerListener {

    private val log = Logger.getInstance(WhipsnakeStartupActivity::class.java)

    override fun projectOpened(project: Project) {
        log.info("Whipsnake: projectOpened fired for ${project.name}")

        val config = WhipsnakeConfigReader.findConfig(project) ?: run {
            log.info("Whipsnake: no [tool.whip] config found, skipping.")
            return
        }
        val basePath = project.basePath ?: return

        log.info("Whipsnake project detected (${config.displayName}), starting setup.")
        ...
        ApplicationManager.getApplication().executeOnPooledThread {
            ...
            if (config.stdlib.isNotEmpty()) {
                val sp = WhipsnakeStubInstaller.install(basePath, config.stdlib, config.board)
    ...
    private fun runSync(
        ...
        log.info("Whipsnake sync: running ${command.joinToString(" ")} in $basePath")
        ...
        try {
            ...
            if (exitCode == 0) {
                log.info("Whipsnake sync succeeded.")
                notifySuccess(project)
            } else {
                log.warn("Whipsnake sync failed (exit $exitCode):\n$output")
            ...
            if (stdlib.isNotEmpty()) {
                val sp = WhipsnakeStubInstaller.install(basePath, stdlib, board)
                refreshAndNotify(project, basePath, sp)
            }
        } catch (e: Exception) {
            log.error("Whipsnake sync error", e)
    ...
    private fun refreshAndNotify(
        ...
        sitePackages?.let { sp ->
            lfs.refreshAndFindFileByNioFile(sp.resolve("whipsnake_circuitpython"))
            lfs.refreshAndFindFileByNioFile(sp.resolve("whipsnake_micropython"))
        }
        ...
        ApplicationManager.getApplication().invokeLater {
            try {
                NotificationGroupManager.getInstance()
                    .getNotificationGroup("Whipsnake")
                    ?.createNotification(
                        "Whipsnake Sync",
