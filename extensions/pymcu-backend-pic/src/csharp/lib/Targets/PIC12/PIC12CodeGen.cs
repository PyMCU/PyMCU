/*
 * -----------------------------------------------------------------------------
 * PyMCU Compiler (pymcuc) — PIC12 (Baseline) Backend  [Experimental]
 ...
 *   - Only 8-bit operations are supported. 16-bit/32-bit/float produce compile-time errors.
 *   - Variable-count shifts produce compile-time errors.
 *   - Multiply, divide, and modulo produce compile-time errors.
 *   - Runtime-indexed flash array access produces a compile-time error.
 *   - No banking support; all data in one bank.
...
public class PIC12CodeGen(DeviceConfig cfg) : CodeGen
{
    ...
    public override void Compile(ProgramIR program, TextWriter output)
    {
        ...
        EmitComment($"Generated by pymcuc for {cfg.Chip} [Experimental PIC12 backend]");
        EmitComment("[WARNING] PIC12 is Experimental. Only 8-bit single-function programs are supported.");
        EmitRaw("");
    ...
    private void CompileInstruction(Instruction instr)
    {
        switch (instr)
        {
            ...
            case BitSet bs:
            {
                ...
                if (addr >= 0)
                {
                    Emit("BSF", Addr(addr), $"{bs.Bit}");
                }
                else
                {
                    // Target not in var map: load address into FSR, then BSF INDF, bit.
                    LoadIntoW(bs.Target);
                    Emit("MOVWF", Addr(0x04));  // FSR = address
                    Emit("BSF", Addr(0x00), $"{bs.Bit}");  // BSF INDF, bit
                }
                break;
            }
            case BitClear bc:
            {
                ...
                if (addr >= 0)
                {
                    Emit("BCF", Addr(addr), $"{bc.Bit}");
                }
                else
                {
                    // Target not in var map: load address into FSR, then BCF INDF, bit.
                    LoadIntoW(bc.Target);
                    Emit("MOVWF", Addr(0x04));  // FSR = address
                    Emit("BCF", Addr(0x00), $"{bc.Bit}");  // BCF INDF, bit
                }
                break;
            }
            case BitCheck bck:
            ...
            case ArrayLoad al:
            {
                ...
                if (al.Index is Constant c)
                    ...
                    throw new NotSupportedException(
                        "PIC12 (Experimental) does not support variable-index ArrayLoad. " +
            ...
            case ArrayStore ast:
            {
                ...
                if (ast.Index is Constant c)
                    ...
                    throw new NotSupportedException(
                        "PIC12 (Experimental) does not support variable-index ArrayStore. " +
                        "Use a constant index.");
                break;
            }
            case LoadIndirect li:
                // PIC12 FSR-based indirect load: load address into FSR (0x04), read INDF (0x00).
                LoadIntoW(li.SrcPtr);
            ...
            case StoreIndirect si:
                // PIC12 FSR-based indirect store: load address into FSR (0x04), write INDF (0x00).
                LoadIntoW(si.DstPtr);
                Emit("MOVWF", Addr(0x04));  // FSR
                LoadIntoW(si.Src);
    ...
    private void CompileUnary(Unary u)
    {
        if (GetValType(u.Dst).SizeOf() == 2)
            throw new NotSupportedException(
                $"PIC12 (Experimental) does not support 16-bit Unary {u.Op}. " +
                "Use uint8 variables only.");
    ...
    private void CompileBinary(Binary b)
    {
        if (GetValType(b.Dst).SizeOf() == 2)
            throw new NotSupportedException(
                $"PIC12 (Experimental) does not support 16-bit Binary {b.Op}. " +
                "Use uint8 variables only.");
        ...
        switch (b.Op)
        {
            ...
            case IrBinOp.Sub:
                ...
                if (src1Addr >= 0)
                {
                    ...
                    else LoadIntoW(b.Src2);
                    Emit("SUBWF", Addr(src1Addr), "W");  // W = src1 - src2 ✓
                }
                else
                {
                    // src1 not in var map: load src1 into TmpLo, then W=src2, SUBWF TmpLo, W
                    LoadIntoW(b.Src1);
                    Emit("MOVWF", Addr(TmpLo));
                    LoadIntoW(b.Src2);
            ...
            case IrBinOp.LShift:
                if (b.Src2 is Constant ls)
                {
                    ...
                    throw new NotSupportedException(
                        "PIC12 (Experimental) does not support variable-count LShift. " +
                        "Use a compile-time constant shift amount.");
                StoreW(b.Dst);
                break;
            ...
            case IrBinOp.RShift:
                if (b.Src2 is Constant rs)
                {
                    ...
                    // Mask out bits that wrapped through carry (rotate leaves MSB from carry).
                    if (rsBits > 0)
                        Emit("ANDLW", $"0x{(0xFF >> rsBits) & 0xFF:X2}");
                }
                else
                    throw new NotSupportedException(
                        "PIC12 (Experimental) does not support variable-count RShift. " +
                        "Use a compile-time constant shift amount.");
                StoreW(b.Dst);
                break;
            ...
            case IrBinOp.Mod:
                throw new NotSupportedException(
                    $"PIC12 (Experimental) does not support {b.Op}. " +
                    "Software multiply/divide routines are not available on PIC12.");
    ...
    private void CompileAugAssign(AugAssign aa)
    {
        if (_varSizes.TryGetValue(
                ...
                out int sz) && sz == 2)
            throw new NotSupportedException(
                $"PIC12 (Experimental) does not support 16-bit AugAssign {aa.Op}. " +
                "Use uint8 variables only.");
        ...
        switch (aa.Op)
        {
            ...
            default:
                EmitComment($"AugAssign {aa.Op} fallback (unimplemented on PIC12)");
                break;
        }
        StoreW(aa.Target);
    }
