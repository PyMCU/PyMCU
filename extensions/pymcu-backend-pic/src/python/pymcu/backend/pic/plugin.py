...
class PicBackendPlugin(BackendPlugin):
    ...
    def get_backend_binary(cls) -> Path:
        ...
        3. backend_root/src/csharp/cli/bin/Debug/net10.0/ (dev: build shortcut).
        4. System PATH.
        """
        ...
        # 2. Development fallback: repo build/bin/.
        # package_dir = .../extensions/pymcu-backend-pic/src/python/pymcu/backend/pic
        repo_root = package_dir.parents[6]
        dev_path = repo_root / "build" / "bin" / binary_name
        if dev_path.exists():
            return dev_path

        # 3. Backend CLI debug build.
        backend_root = package_dir.parents[4]  # .../extensions/pymcu-backend-pic
        runner_debug = (
