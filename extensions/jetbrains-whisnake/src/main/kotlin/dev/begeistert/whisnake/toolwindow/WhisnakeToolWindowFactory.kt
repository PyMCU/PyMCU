package dev.begeistert.whisnake.toolwindow
...
/**
 * Factory for the "Whisnake" tool window (anchor=bottom).
 *
 ...
 */
class WhisnakeToolWindowFactory : ToolWindowFactory {

    override fun createToolWindowContent(project: Project, toolWindow: ToolWindow) {
        val panel = WhisnakeToolWindowPanel(project)
...
private class WhisnakeToolWindowPanel(private val project: Project) : JPanel(BorderLayout()) {

    private val log = Logger.getInstance(WhisnakeToolWindowPanel::class.java)

    private val chipLabel = JLabel("No Whisnake project detected")
    ...
    private fun refreshChipLabel() {
        val config = WhisnakeConfigReader.findConfig(project)
        if (config == null) {
            chipLabel.text = "No Whisnake project detected"
            return
        }
    ...
    private fun runCommand(command: String) {
        ...
        ApplicationManager.getApplication().executeOnPooledThread {
            try {
                ...
                log.error("Whisnake toolwindow command error", e)
            }
        }
    }
    ...
    private fun runSync() {
        ...
        ApplicationManager.getApplication().executeOnPooledThread {
            try {
                ...
                log.error("Whisnake toolwindow sync error", e)
            }
        }
    }
