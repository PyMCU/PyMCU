...
/**
 * Adds a "Whisnake" entry to PyCharm's New Project wizard.
 *
 ...
 */
class WhisnakeProjectGenerator : DirectoryProjectGenerator<WhisnakeNewProjectSettings> {

    private val log = Logger.getInstance(WhisnakeProjectGenerator::class.java)

    override fun getName(): String = "Whisnake"

    override fun getDescription(): String =
        "Create a Whisnake project targeting an AVR or PIC microcontroller."
    ...
    override fun createPeer(): ProjectGeneratorPeer<WhisnakeNewProjectSettings> =
        WhisnakeProjectGeneratorPeer()

    override fun generateProject(
        project: Project,
        baseDir: VirtualFile,
        settings: WhisnakeNewProjectSettings,
        module: Module
    ) {
        ...
        ApplicationManager.getApplication().runWriteAction {
            try {
                ...
                log.info("Whisnake project scaffolded at $basePath")
            } catch (e: Exception) {
                log.error("Failed to scaffold Whisnake project", e)
            }
        }
    ...
    private fun buildPyproject(name: String, s: WhisnakeNewProjectSettings): String {
        // board OR chip line
    ...
    private fun buildMainPy(s: WhisnakeNewProjectSettings): String {
    ...
    /** Bare Whisnake template using pymcu.hal directly. */
    ...
    /** Bare Whisnake template for non-AVR chips. */
    private fun buildGenericTemplate(chip: String): String = """
        # Whisnake — target: $chip
        from pymcu.types import uint8
        from pymcu.time import delay_ms
    ...
    private fun buildMicroPythonTemplate(board: String?, chip: String): String {
        val target = if (board != null) "$board ($chip)" else chip
        return """
            # Whisnake — target: $target  [MicroPython compat]
    ...
    private fun buildCircuitPythonTemplate(board: String?, chip: String): String {
        val target = if (board != null) "$board ($chip)" else chip
        return """
            # Whisnake — target: $target  [CircuitPython compat]
    ...
    private fun runSync(basePath: String, packageManager: String) {
        ...
        log.info("Whisnake new-project sync: ${command.joinToString(" ")} in $basePath")
        try {
            ...
            log.warn("Whisnake sync after project creation failed", e)
        }
    }
}
