...
/**
 * A single Whisnake run configuration that runs one of: build, flash, clean.
 */
@Suppress("UnstableApiUsage")
class WhisnakeRunConfiguration(
    project: Project,
    ...
    override fun getConfigurationEditor(): SettingsEditor<WhisnakeRunConfiguration> =
        WhisnakeRunConfigurationEditor()
    ...
    private inner class WhisnakeRunConfigurationEditor : SettingsEditor<WhisnakeRunConfiguration>() {

        private val commandCombo = ComboBox(arrayOf("build", "flash", "clean"))

        override fun resetEditorFrom(config: WhisnakeRunConfiguration) {
            commandCombo.selectedItem = config.command
        }

        override fun applyEditorTo(config: WhisnakeRunConfiguration) {
