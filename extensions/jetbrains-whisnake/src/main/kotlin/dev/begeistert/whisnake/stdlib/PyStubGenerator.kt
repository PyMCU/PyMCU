...
/**
 * Generates PEP 561 `.pyi` stub files by parsing Whisnake stdlib `.py` source files.
 *
 ...
 *  - `pass` → `...`
 *  - Whisnake-specific types (`uint8`, `uint16`, `const[str]`, …) → standard Python types
...
object PyStubGenerator {
    ...
    fun generateFrom(sourceFile: Path): String? = try {
        ...
        log.warn("Whisnake stubs: failed to generate stub from $sourceFile", e)
        null
    }
    ...
    private fun generate(lines: List<String>, fileName: String): String {
        ...
        sb.append("# Whisnake — generated stub for $fileName (do not edit manually)\n")
        var i = 0
        val n = lines.size
     ...
     *  - Maps Whisnake types to standard Python types
