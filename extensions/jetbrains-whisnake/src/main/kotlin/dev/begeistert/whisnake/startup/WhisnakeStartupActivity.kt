package dev.begeistert.whisnake.startup
...
import dev.begeistert.whisnake.settings.WhisnakeSettings
 ...
 *
 * When a [tool.whip] section is detected in pyproject.toml it:
...
class WhisnakeStartupActivity : ProjectManagerListener {

    private val log = Logger.getInstance(WhisnakeStartupActivity::class.java)

    override fun projectOpened(project: Project) {
        log.info("Whisnake: projectOpened fired for ${project.name}")

        val config = WhisnakeConfigReader.findConfig(project) ?: run {
            log.info("Whisnake: no [tool.whip] config found, skipping.")
            return
        }
        val basePath = project.basePath ?: return

        log.info("Whisnake project detected (${config.displayName}), starting setup.")
        ...
        ApplicationManager.getApplication().executeOnPooledThread {
            ...
            if (config.stdlib.isNotEmpty()) {
                val sp = WhisnakeStubInstaller.install(basePath, config.stdlib, config.board)
    ...
    private fun runSync(
        ...
        log.info("Whisnake sync: running ${command.joinToString(" ")} in $basePath")
        ...
        try {
            ...
            if (exitCode == 0) {
                log.info("Whisnake sync succeeded.")
                notifySuccess(project)
            } else {
                log.warn("Whisnake sync failed (exit $exitCode):\n$output")
            ...
            if (stdlib.isNotEmpty()) {
                val sp = WhisnakeStubInstaller.install(basePath, stdlib, board)
                refreshAndNotify(project, basePath, sp)
            }
        } catch (e: Exception) {
            log.error("Whisnake sync error", e)
    ...
    private fun refreshAndNotify(
        ...
        sitePackages?.let { sp ->
            lfs.refreshAndFindFileByNioFile(sp.resolve("whisnake_circuitpython"))
            lfs.refreshAndFindFileByNioFile(sp.resolve("whisnake_micropython"))
        }
