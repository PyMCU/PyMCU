...
| Member access `obj.x` / method calls `obj.m()` | Inline expansion; zero SRAM. A class attribute whose class defines `__get__`/`__set__` is a descriptor (#360); `obj` in those methods is the owning instance even when annotated with a typing-only placeholder (#419); `type(inst)` in the rewrite is the source class name, including for an imported class; `value: Any` on `__set__` is the written value, not a missing width; `value <<= n` keeps `value` as a local so a later `reg \|= value` still sees the shifted bits |
...
| `for v in obj` over `__getitem__` | The sequence protocol, when `__len__` is a compile-time constant: the trip count is known, so it unrolls to one `__getitem__` per index and costs what `for i in range(len(obj))` costs. The constant may be a literal, a module constant, a class attribute, or one method hop away (`self._size()`, `Eeprom().size()`), so a chip fact can stay in the HAL. A run-time `__len__` (or none) is refused, naming `__getitem__` and the missing trip count. `__iter__`/`__next__` stay unsupported: `StopIteration` is an exception, so the loop would have nothing to end on |
...
| Closed dict/set literals | `d = {0: 10, "mid": 2}` / `OK = {1, 3, 5}` bind compile-time lookup tables (no storage): `d[const]` folds, `d[runtime]` lowers to a compare chain raising `KeyError` (catchable), `x in d` / `x in {...}` membership chains, `len(d)` folds. A class-body dict (`self.gain_values[gain]`) is the same table, including mixed int/float values. String keys fold on constant lookups. Read-only — for mutation use `FixedDict` |
