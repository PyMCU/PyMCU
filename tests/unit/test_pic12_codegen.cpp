...
TEST(PIC12CodeGenTest, SimpleReturn) {
  DeviceConfig cfg{.chip = "pic12f683"};
  cfg.chip = "10f200";
...
TEST(PIC12CodeGenTest, NoAddLW_Optimization) {
  ...
  // a = b + 10
  func.body.push_back(tacky::Binary{tacky::BinaryOp::Add, tacky::Variable{"b"},
...
TEST(PIC12CodeGenTest, TRISInstruction) {
  ...
  // TRISGPIO (0x86) = 0
  func.body.push_back(
      tacky::Copy{tacky::Constant{0}, tacky::MemoryAddress{0x86}});
  program.functions.push_back(func);
...
TEST(PIC12CodeGenTest, NegationNoHardcoded) {
  ...
  // x = -y
  func.body.push_back(tacky::Unary{tacky::UnaryOp::Neg, tacky::Variable{"y"},
                                   tacky::Variable{"x"}});
  program.functions.push_back(func);
