...
TEST(PIOCodeGenTest, SimpleMove) {
  ...
  // x = 5
  func.body.push_back(tacky::Copy{tacky::Constant{5}, tacky::Variable{"x"}});
...
TEST(PIOCodeGenTest, JumpIfZero) {
  ...
  // if x == 0 goto label
  func.body.push_back(tacky::JumpIfZero{tacky::Variable{"x"}, "mylabel"});
  program.functions.push_back(func);
...
TEST(PIOCodeGenTest, BitNot) {
  ...
  // y = ~x
  func.body.push_back(tacky::Unary{tacky::UnaryOp::BitNot, tacky::Variable{"x"},
                                   tacky::Variable{"y"}});
  program.functions.push_back(func);
...
TEST(PIOCodeGenTest, DecrementOnly) {
  ...
  // x = x - 1
  func.body.push_back(tacky::Binary{tacky::BinaryOp::Sub, tacky::Variable{"x"},
                                    tacky::Constant{1}, tacky::Variable{"x"}});
  program.functions.push_back(func);
...
TEST(PIOCodeGenTest, Intrinsics) {
  ...
  func.body.push_back(tacky::Call{
      "__pio_wait",
      {tacky::Constant{1}, tacky::MemoryAddress{1}, tacky::Constant{0}},
      std::monostate{}});
  // delay(5) on previous instruction
  func.body.push_back(
      tacky::Call{"delay", {tacky::Constant{5}}, std::monostate{}});
...
TEST(PIOCodeGenTest, UnsupportedOpsThrow) {
  ...
  func.body.push_back(tacky::Binary{tacky::BinaryOp::Add, tacky::Variable{"x"},
                                    tacky::Constant{1}, tacky::Variable{"x"}});
  program.functions.push_back(func);
