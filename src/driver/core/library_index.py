...
def build_example_assembly(lib: Library, chip: str, *, pymcu: Path,
                           example: str = "", env_paths: list[str] | None = None,
                           max_lines: int = 2_000) -> dict:
    """
    Compile the library's example for *chip* and hand back the assembly.

    Generated on request rather than stored in the index: a firmware's assembly
    runs to thousands of lines, and it is only true for one chip and one
    compiler version -- exactly the two things that change underneath a
    published file. Compiling it here means what is shown is what this machine
    would flash.
    """
    example_dir = lib.example_dir(example)
    if example_dir is None:
        return {"ok": False, "error": "This library ships no example to compile."}
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        config = work / "pyproject.toml"
        if not config.exists():
            return {"ok": False, "error": "The example has no pyproject.toml."}
        ...
        if result.returncode != 0:
            detail = (result.stderr or result.stdout or "").strip().splitlines()
            return {"ok": False, "error": detail[-1][:300] if detail else "The build failed."}

        # Backends name their output differently (firmware.asm, firmware.gas.asm),
        # so take what the build actually produced.
        candidates = sorted((work / "dist").glob("*.asm"))
        if not candidates:
            return {"ok": False, "error": "The build produced no assembly for this target."}
        ...
        flash, _ram = _parse_flash(result.stdout)
        return {
            "ok": True,
            "chip": chip,
