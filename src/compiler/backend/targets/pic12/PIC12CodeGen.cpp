...
PIC12CodeGen::PIC12CodeGen(DeviceConfig cfg) : config(std::move(cfg)), ram_head(0x08) {}
...
void PIC12CodeGen::emit(const std::string& mnemonic) const {
    const_cast<PIC12CodeGen*>(this)->assembly.push_back(PIC12AsmLine::Instruction(mnemonic));
}
void PIC12CodeGen::emit(const std::string& mnemonic, const std::string& op1) const {
    const_cast<PIC12CodeGen*>(this)->assembly.push_back(PIC12AsmLine::Instruction(mnemonic, op1));
}
void PIC12CodeGen::emit(const std::string& mnemonic, const std::string& op1, const std::string& op2) const {
    const_cast<PIC12CodeGen*>(this)->assembly.push_back(PIC12AsmLine::Instruction(mnemonic, op1, op2));
}
void PIC12CodeGen::emit_label(const std::string& label) const {
    const_cast<PIC12CodeGen*>(this)->assembly.push_back(PIC12AsmLine::Label(label));
}
void PIC12CodeGen::emit_comment(const std::string& comment) const {
    const_cast<PIC12CodeGen*>(this)->assembly.push_back(PIC12AsmLine::Comment(comment));
}
...
void PIC12CodeGen::load_into_w(const tacky::Val& val) {
    if (const auto c = std::get_if<tacky::Constant>(&val)) {
        ...
        if (addr == "0x85") addr = get_or_alloc_variable("__tris_shadow_5");
        else if (addr == "0x86") addr = get_or_alloc_variable("__tris_shadow_6");
        else if (addr == "0x87") addr = get_or_alloc_variable("__tris_shadow_7");
...
void PIC12CodeGen::store_w_into(const tacky::Val& val) {
    ...
    int tris_reg = -1;
    if (addr == "0x85") tris_reg = 5;
    else if (addr == "0x86") tris_reg = 6;
    else if (addr == "0x87") tris_reg = 7;

    if (tris_reg != -1) {
        emit("TRIS", std::to_string(tris_reg));
        // Shadow it for future reads/bit-ops
        std::string shadow = get_or_alloc_variable(std::format("__tris_shadow_{}", tris_reg));
        emit("MOVWF", shadow);
        return;
    }
...
void PIC12CodeGen::compile_variant(const tacky::Binary& arg) {
    ...
    switch (arg.op) {
        ...
        case tacky::BinaryOp::BitAnd:
            if (std::holds_alternative<tacky::Constant>(arg.src2)) emit("ANDLW", right);
            else emit("ANDWF", right, "W");
            break;
        case tacky::BinaryOp::BitOr:
            if (std::holds_alternative<tacky::Constant>(arg.src2)) emit("IORLW", right);
            else emit("IORWF", right, "W");
            break;
        case tacky::BinaryOp::BitXor:
            if (std::holds_alternative<tacky::Constant>(arg.src2)) emit("XORLW", right);
