...
static std::vector<PIC14AsmLine>
coalesce_bit_ops(const std::vector<PIC14AsmLine> &lines) {
...
std::vector<PIC14AsmLine>
PIC14Peephole::optimize(const std::vector<PIC14AsmLine> &lines) {
        ...
        for (size_t i = 0; i < result.size(); ++i) {
            ...
            if (current.mnemonic == "MOVLW") {
                ...
                for (int j = (int) next.size() - 1; j >= 0; --j) {
                    if (next[j].type == PIC14AsmLine::LABEL)
                        break;
