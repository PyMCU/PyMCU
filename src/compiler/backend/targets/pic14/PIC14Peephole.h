...
struct PIC14AsmLine {
    ...
    static PIC14AsmLine Instruction(std::string m, std::string o1 = "", std::string o2 = "") {
    ...
    static PIC14AsmLine Raw(std::string r) {
        return {RAW, "", "", "", "", r};
    }
    static PIC14AsmLine Empty() {
        return {EMPTY, "", "", "", "", ""};
    }
...
class PIC14Peephole {
public:
    static std::vector<PIC14AsmLine> optimize(const std::vector<PIC14AsmLine>& lines);
};
