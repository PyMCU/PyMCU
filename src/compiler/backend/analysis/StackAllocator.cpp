...
StackAllocator::allocate(const tacky::Program &program) {
    offsets.clear();
    offsets_base.clear();
    call_graph.clear();
    global_names.clear();
    ...
    int global_offset = 0;
    for (const auto &global_name: program.globals) {
        offsets[global_name] = global_offset;
        global_names.insert(global_name);
        global_offset++;
    }
...
void StackAllocator::calculate_offsets(const std::string &func_name,
    ...
    for (const auto &var_name: node.locals) {
        if (global_names.contains(var_name))
            continue;
