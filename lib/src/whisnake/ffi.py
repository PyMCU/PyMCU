...
#
# Requires: [tool.whisnake.assembler] = "avr-as"  (or automatic detection when
# [tool.whisnake.ffi] is present -- the build driver switches to the avr-as
# toolchain automatically when C sources are declared).

# @extern("symbol") is recognized syntactically by the compiler in
# parseFunction(). This stub exists so 'from whisnake.ffi import extern'
