...
def wdt_arm_rt(timeout_ms: uint16):
    # Arm the watchdog in reset mode from a RUNTIME timeout (ms).
    # Unlike wdt_enable(const), the timeout need not be known at compile time:
    # a plain runtime if/elif picks the prescaler bucket and each branch calls
