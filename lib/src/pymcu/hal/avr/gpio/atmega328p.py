...
# Non-inline asm() helpers with counted inner loops.
# Loop: SBIS/SBIC(1) + RJMP(2) + ADIW(2) + CP/CPC(2) + BRCS(1) = 9 cyc/iter, not 8: the
# inner RJMP is taken on the branch a running loop is actually on (still counting), so it
# and BRCS both cost their TAKEN price every iteration but one. 9 cycles at 16 MHz is
# 16/9 = 1.778 iterations per microsecond, not the 2/us a plain count>>1 return assumes --
# PyMCU/PyMCU#407, confirmed at a fixed 8/9 ratio across four independent pulse widths.
# mode=0 (wait): returns 1 if condition met within timeout, 0 on timeout (no scaling: not
# a duration).
# mode=1 (meas): returns approximately loop-count * 9/16 (count>>1 plus count>>4, a cheap
# fixed-point stand-in for the exact ratio that costs one extra 16-bit register pair and
# no division) when condition met, the same computed from max_count on timeout.
