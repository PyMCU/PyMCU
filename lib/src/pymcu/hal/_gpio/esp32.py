...
#   a2 -- holds the 32-bit value to store (mask or data); set before asm()
#   a3 -- holds the MMIO register address; set before asm()
#   MMIO write: s32i a2, a3, 0  -- stores *(a3+0) = a2
#   MMIO read:  l32i a2, a3, 0  -- loads  a2 = *(a3+0)
...
def gpio_set_mode(pin: uint32, mode: uint32):
    ...
    if mode == OUT:
        ...
        # a2=mask, a3=GPIO_ENABLE_W1TS: set the bit to enable output.
        asm("s32i a2, a3, 0")
    else:
        mask: uint32 = 1 << pin
        ptr_dis: uint32 = GPIO_ENABLE_W1TC
        # a2=mask, a3=GPIO_ENABLE_W1TC: set the bit to disable output (float/input).
        asm("s32i a2, a3, 0")
...
def gpio_write(pin: uint32, value: uint32):
    ...
    if value != 0:
        ...
        # a2=mask, a3=GPIO_OUT_W1TS: write 1 to set the output bit high.
        asm("s32i a2, a3, 0")
    else:
        ptr_clr: uint32 = GPIO_OUT_W1TC
        # a2=mask, a3=GPIO_OUT_W1TC: write 1 to clear the output bit (set low).
        asm("s32i a2, a3, 0")
...
def gpio_read(pin: uint32) -> uint32:
    val: uint32 = 0
    ptr_in: uint32 = GPIO_IN
    # a3=GPIO_IN: load all 32 input levels into a2; result goes into val.
    asm("l32i a2, a3, 0")
