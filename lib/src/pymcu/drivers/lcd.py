...
class LCD:
    ...
    @inline
    def __init__(self: uint8, rs: const[str], en: const[str], d4: const[str], d5: const[str], d6: const[str], d7: const[str]):
        self._rs = rs
        self._en = en
        self._d4 = d4
        self._d5 = d5
    ...
    @inline
    def print_str(self: uint8, s: const[str]):
        match __CHIP__.arch:
            case "avr":
    ...
    @inline
    def set_cursor(self: uint8, col: uint8, row: uint8):
        match __CHIP__.arch:
            case "avr":
    ...
    @inline
    def write_char(self: uint8, c: uint8):
        match __CHIP__.arch:
            case "avr":
